...
mapel_name = "Mapel Pilihan dan Sistem Kendali Elektronika"
tingkat = "XI"

print(f"[*] Mengunggah {len(data_tp)} Tujuan Pembelajaran (TP) ke Cloud Firestore...")

count = 0
for meeting, tp_text in data_tp:
    doc_id = f"tp_ske_xi_p{meeting:02d}"
    url = f"https://firestore.googleapis.com/v1/projects/{PROJECT_ID}/databases/(default)/documents/{COLLECTION}/{doc_id}?key={API_KEY}"
    
    payload = {
        "fields": {
            "mapel": {"stringValue": mapel_name},
            ...
            "kategori": {"stringValue": "Embedded System & Otomasi"}
        }
    }
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            if resp.status in (200, 201):
                count += 1
                print(f"  [OK] Pertemuan {meeting:02d}: Tersimpan -> {doc_id}")
    except Exception as e:
        print(f"  [ERR] Gagal unggah Pertemuan {meeting}: {e}")

print(f"\n[+] Selesai! Berhasil mengunggah {count}/{len(data_tp)} data TP ke Cloud Firestore.")
